...
const navItems: NavItem[] = [
  ...
  },
  {
    ...
    children: [
      { to: '/accounts/deposit', label: 'Depósito', roles: ['admin', 'operator'] }
    ]
  },
  {
    to: '/admin/users',
    ...
    <div className="min-h-screen bg-slate-100 text-slate-900 transition dark:bg-slate-950 dark:text-slate-100">
      <div className="flex min-h-screen">
        ...
        >
          ...
          <div className={`flex items-center border-b border-slate-200 dark:border-slate-800 p-4 ${isSidebarCollapsed ? 'justify-center' : 'gap-3'}`}>
            ...
            {!isSidebarCollapsed && (
              <div className="flex-1 w-2/3">
                ...
                <p className="text-[10px] text-slate-500 dark:text-slate-400 leading-tight mt-0.5">INTELIGÊNCIA FINANCEIRA PARA CRÉDITO SEGURO</p>
              </div>
            )}
          </div>
              ...
              {allowedNavItems.map((item) => {
                ...
                return (
                  <div key={item.to}>
                    ...
                    {hasChildren && isExpanded && !isSidebarCollapsed && allowedChildren && allowedChildren.length > 0 && (
            ...
            <div className="flex h-16 items-center justify-between px-4 md:px-8">
              <div className="flex items-center gap-2">
                ...
                <button
                  ...
                  className="hidden md:inline-flex h-10 w-10 items-center justify-center rounded-lg border border-slate-200 bg-white text-slate-600 shadow-sm transition hover:bg-slate-50 focus:outline-none focus:ring-2 focus:ring-blue-500 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-100"
                ...
                <button
                  type="button"
                  className="inline-flex h-10 w-10 items-center justify-center rounded-lg border border-slate-200 bg-white text-slate-600 shadow-sm transition hover:bg-slate-50 focus:outline-none focus:ring-2 focus:ring-blue-500 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-100 md:hidden"
      ...
      {isUserSidebarOpen && (
        <div className="fixed inset-0 z-50 flex items-start justify-end">
          <div className="absolute inset-0 bg-slate-900/50 backdrop-blur-sm" onClick={() => setIsUserSidebarOpen(false)} />
          <div
            ref={userSidebarRef}
            role="dialog"
            aria-modal="true"
